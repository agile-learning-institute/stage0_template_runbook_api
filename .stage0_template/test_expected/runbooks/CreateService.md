...
**SERVICE_NAME:** The runbook clones the https://github.com/agile-learning-institute/mentorhub repo to get design specifications. `Specifications/architecture.yaml` and `Specifications/product.yaml` are used. `SERVICE_NAME` is a domain name that maps to a section of architecture.yaml. Repos within that domain with `type: api` or `type: spa` are processed; `type: spa_ref` repos are skipped. Created repo names are prefixed with `info.slug` from product.yaml (e.g. `mentorhub_mongodb_api`). Docker image names come from each repo's `image` field in architecture.yaml; repos without `image` (e.g. libraries) are created and merged but not built or pushed. Organization and registry for git and registry login are read from `Specifications/product.yaml`.
